package Assingement;

import org.openqa.selenium.By;
import org.openqa.selenium.chrome.ChromeDriver;

public class ToUseIdLocator {

	public static void main(String[] args) throws InterruptedException {
		
		ChromeDriver driver = new ChromeDriver();
		driver.manage().window().maximize();
		driver.get("https://www.actitime.com/free-online-trial");
		Thread.sleep(3000);
		driver.findElement(By.id("FirstName")).sendKeys("Honysahu");
		Thread.sleep(3000);
		driver.findElement(By.id("LastName")).sendKeys("123456");
		Thread.sleep(3000);
        driver.findElement(By.id("Email")).sendKeys("Honeysahu@gmail.com");
        Thread.sleep(3000);
        driver.findElement(By.id("Company")).sendKeys("Samsung");
        Thread.sleep(3000);
        driver.findElement(By.id("confirm-button"));
	}

}
